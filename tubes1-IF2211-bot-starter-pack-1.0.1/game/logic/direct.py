...
class DirectLogic(BaseLogic):
    ...
    def next_move(self, board_bot: GameObject, board: Board):
        ...
        if (self.board_bot.position == self.static_temp_goals):
            self.static_temp_goals = None
                ...
                # Di next_move, modifikasi kondisi untuk pergi ke markas:
        # props = board_bot.properties (sudah didefinisikan)
        # current_position = board_bot.position (sudah didefinisikan)

        enemy_is_near_and_risky = False
        if props.diamonds >= 3: # Gunakan >= 3 sesuai "memiliki 3 berlian"
            for an_enemy in self.enemy:
                dist_to_enemy = abs(an_enemy.position.x - current_position.x) + abs(an_enemy.position.y - current_position.y)
                if dist_to_enemy <= 2:
                    enemy_is_near_and_risky = True
                    break

        # Gabungkan dengan kondisi kembali ke markas yang sudah ada
        should_go_to_base = props.diamonds == 5 or \
                            (props.milliseconds_left < 5000 and props.diamonds >= 2) or \
                            enemy_is_near_and_risky

        # Kondisi tambahan dari kode asli: (self.calculate_near_base() and props.diamonds > 2)
        if not should_go_to_base and self.calculate_near_base() and props.diamonds > 2:
            should_go_to_base = True
            # Jika kembali ke markas karena dekat & >2 berlian, tidak perlu reset via_teleporter spesifik
            # Namun, jika find_best_way_to_base memutuskan TIDAK via teleporter, maka static_goals dll harus direset

        if should_go_to_base:
            self.goal_position = self.find_best_way_to_base()
            # Reset goal non-base hanya jika kita tidak secara eksplisit diarahkan via teleporter ke base
            # find_best_way_to_base sendiri sudah mengatur self.static_direct_to_base_via_teleporter
            # dan self.static_goals jika memilih teleporter.
            if not self.static_direct_to_base_via_teleporter:
                self.static_goals = [] # Hapus target berlian/lainnya
                self.static_goal_teleport = None
                self.static_temp_goals = None # Hapus manuver penghindaran sementara
        else:
            # Logika untuk mencari berlian
            if not self.static_goals: # Periksa apakah static_goals kosong, bukan len() == 0 untuk konsistensi
                self.find_nearest_diamond()
            if self.static_goals: # Pastikan static_goals tidak kosong setelah find_nearest_diamond
                self.goal_position = self.static_goals[0]

        if self.static_temp_goals: # Jika ada tujuan sementara, gunakan itu
    ...
    # Dalam find_nearest_red_button
    def find_nearest_red_button(self):
        if not self.redButton:
            return float("inf"), None # Kembalikan jarak sangat besar dan tanpa posisi
        # ... sisa logika yang ada
        current_position = self.board_bot.position
        # Pertimbangkan iterasi jika ada beberapa tombol merah dan memilih yang benar-benar terdekat
        # Untuk saat ini, asumsikan hanya ada satu atau yang pertama selalu menjadi target:
        btn = self.redButton[0]
        distance = abs(btn.position.x - current_position.x) + abs(btn.position.y - current_position.y)
        return distance, btn.position
    ...
    # Dalam find_other_teleport
    def find_other_teleport(self, teleport: GameObject):
        for t_obj in self.teleporter: # Ganti nama variabel 't' agar tidak bentrok jika ada variabel global/kelas 't'
            if t_obj.id != teleport.id:
                return t_obj.position
        return None # Kembalikan None jika tidak ada teleporter lain ditemukan (misalnya, hanya ada satu)
